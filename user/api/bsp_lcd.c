...
/**
  ...
  * @retval None
  */
void LCD_DrawLine(uint8_t Xpos, uint16_t Ypos, uint16_t Length, uint8_t Direction,uint16_t TextColor)
{
  uint32_t i = 0;
...
void LCD_showchinese(uint16_t x, uint16_t y,uint8_t *p,uint16_t color)
{
	uint16_t cnt=0; uint16_t x0=x;
  for(uint8_t i=0;i<32;i++)
	{
	  for(uint8_t j=0;j<8;j++)
		{
//		 if(hz[i]&0x80) //判读像素点是否点亮  1还是0
//		 {
...
//		 {}
			...
			if((p[i]&(1<<(7-j)))!=0)
			{
			 LCD_Fast_DrawPoint(x+cnt%16,cnt/16+y,color);
			}else
			{
			LCD_Fast_DrawPoint(x+cnt%16,cnt/16+y,Backcolor);
			}
			cnt++;
	}
...
0x17,0xFE,0x11,0x08,0xE2,0x48,0x22,0x28,0x23,0xFC,0x20,0x08,0x20,0x50,0x00,0x20},/*"海",1*/

{0x00,0x40,0x20,0x40,0x17,0xFC,0x10,0x40,0x83,0xF8,0x40,0x80,0x47,0xFE,0x10,0x90,
0x11,0x10,0x21,0xFE,0xE2,0x10,0x22,0x90,0x24,0x50,0x28,0x10,0x20,0x50,0x00,0x20},/*"涛",2*/
};

void LCD_ShowChineseStr(uint16_t x, uint16_t y,  uint16_t Color, char str[])
{
	uint16_t i=0;
	while(str[i] != '\0')//遍历字符串
	{
		...
		while(hz1616[j] != '\0')
		{
			...
			{
				LCD_showchinese(x, y, Hz_16_16[j/2],RED);
				x+=16;
				if(lcddev.width-x<16) {	//换行处理
					y+=16;
